package com.sp.cjgc.core.utils.order;

import com.sp.cjgc.common.constants.AppConstants;
import com.sp.cjgc.common.domain.CarVisitor;
import com.sp.cjgc.common.domain.MonthlyInsurancePayment;
...
import com.sp.cjgc.common.enums.CarGroupEnum;
import com.sp.cjgc.common.enums.OrderStatus;
import com.sp.cjgc.common.enums.RespEnum;
import com.sp.cjgc.common.exception.BizException;
import com.sp.cjgc.common.utils.DateTimeUtil;
...
public class ExitParkingUtil {
            ...
            if (Objects.nonNull(insurancePayment)) {
                ...
                // 既不是月保车、又不是访客车，就按照临停车计费
                    ...
                    if (Objects.nonNull(parkCollectCoupons) && parkCollectCoupons.getIsPlay() == 1) {
                        ...
                        if (playTime.plusMinutes(AppConstants.TIME).isBefore(nowTime)) {
                            // 超出30分钟，不开闸
    ...
    public static OrderPaidCatOutbound createOrder(String license, String deviceIp) {
        ...
        if (Objects.nonNull(parkCollectCoupons) && parkCollectCoupons.getIsPlay() == 1) {
            ...
            LocalDateTime playTime = parkCollectCoupons.getPlayTime();
            // 判断订单支付时间 + 30分钟以后，是否超出了当前系统时间
            if (playTime.plusMinutes(AppConstants.TIME).isBefore(LocalDateTime.now())) {
                // 根据用户领劵信息创建预支付订单
