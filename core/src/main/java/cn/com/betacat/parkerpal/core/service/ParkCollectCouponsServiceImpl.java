...
public class ParkCollectCouponsServiceImpl
    ...
    public synchronized ParkCollectCoupons userCollectCoupons(String userId, String licensePlates, Long timestamp) {
        ...
        // 获取临保收费标准，单位：元/小时
        String tollStandardStr = chargeRulesService.getTollStandard();
        // 判断是否存在临保收费标准
        ...
        // 计算停车时长
        parkCollectCoupons.setTotalDuration(temporaryService.getParkingDuration(startTime, now));
        // 计算停车总金额、优惠金额、优惠时长、优惠信息并保存保存数据
        temporaryService.computationalCost(parkCollectCoupons, users.getFreeTime(), tollStandardStr);
